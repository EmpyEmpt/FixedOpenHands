...
class ClassificationModel(InferenceModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        acc = self.accuracy_metric(F.softmax(y_hat, dim=-1), batch["labels"], task = 'multiclass', num_classes= self.num_classes)
        # acc = self.accuracy_metric(F.softmax(y_hat, dim=-1), batch["labels"], task = 'multiclass', num_classes= 2000)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        acc_top1 = self.accuracy_metric(preds, batch["labels"], task = 'multiclass', num_classes= self.num_classes)
        acc_top3 = self.accuracy_metric(preds, batch["labels"], task = 'multiclass', num_classes= self.num_classes, top_k=3)
        acc_top5 = self.accuracy_metric(preds, batch["labels"], task = 'multiclass', num_classes= self.num_classes, top_k=5)
        # acc_top1 = self.accuracy_metric(preds, batch["labels"], task = 'multiclass', num_classes= 2000)
        # acc_top3 = self.accuracy_metric(preds, batch["labels"], task = 'multiclass', num_classes= 2000, top_k=3)
        # acc_top5 = self.accuracy_metric(preds, batch["labels"], task = 'multiclass', num_classes= 2000, top_k=5)
