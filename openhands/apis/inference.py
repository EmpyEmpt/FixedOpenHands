...
class InferenceModel(pl.LightningModule):
    ...
    def __init__(self, cfg, stage="test"):
        ...
        self.num_classes = self.datamodule.num_class
    ...
    def init_from_checkpoint_if_available(self, map_location=torch.device("cpu")):
        ...
        #       Hack for fine-tuning pretrained transformer 
        # self.model.decoder.l2 = nn.Linear(in_features=96, out_features=2000)
        
        #       Hack for fine-tuning pretrained st-gcn
        # n_features = self.model.decoder.n_features
        # requires_grad(self.model, False)
        # self.model.decoder.classifier = nn.Linear(n_features, 133)
